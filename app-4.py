...
if st.session_state.current_image:
    ...
    if button_clicked:
        try:
            ...
            # Stream with native Streamlit — smooth typing animation
    ...
    # 2. STATIC RENDERING — on every rerun (download click, slider change, etc.)
    elif st.session_state.analysis_result and st.session_state.analysis_mode == mode_key:
        tag_class = TAG_CLASSES[mode_key]
        tag_text  = TAGS[mode_key]

        # Render custom header card only (no text inside HTML — avoids markdown-in-div trap)
        st.markdown(f"""
        ...
        # Render text natively — bold, lists, line breaks all work correctly
        with st.container(border=True):
            st.markdown(st.session_state.analysis_result)

    # 3. DOWNLOAD BUTTON — always visible when result exists, safe from rerun cycle
